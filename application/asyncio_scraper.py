...
class Scraper:
    ...
    _ERRORS = (ClientPayloadError, ServerDisconnectedError, TimeoutError)
    ...
    def parse_car_details(html: str) -> dict[str, Any]:
        ...
        phone_number = soup.find('span', class_='mhide')
        ...
        return {
            ...
            'phone_number': phone_number.get_text() if phone_number is not None else 'None',
    ...
    async def get_cars_details(self, session: aiohttp.ClientSession,
                               page_id: int,
                               semaphore: asyncio.Semaphore) -> list[str] or None:
    ...
    async def aiohttp_event_loop(self):
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            tasks = [asyncio.create_task(self.get_cars_details(session=session, page_id=page_id, semaphore=semaphore))
